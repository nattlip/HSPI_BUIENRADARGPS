...
Public Class plugin
    '' Public sConfigPage As String = "Sample_Config"
    'Public sStatusPage As String = "Sample_Status"
    Public sBuienRadarPage As String = "BuienRadarGps"
    '' Public sRegisterConfigPage As String = "BuienRadarConfig"
    'Public sEmptyPage As String = "Empty"
    '' Public ConfigPage As New web_config(sConfigPage)
    'Public StatusPage As New web_status(sStatusPage)
    Public BuienRadarPage As New web_BuienRadarConfig(sBuienRadarPage)
    '' Public BuienRadarConfigPage As New web_BuienRadarConfig(sRegisterConfigPage)
    'Public EmptyPage As New web_empty(sEmptyPage)
...
#Region "Device Interface"
    ...
    Public Function ConfigDevice(ref As Integer, user As String, userRights As Integer, newDevice As Boolean) As String
        ...
        Dim ic As Integer = PED.UnNamedCount
        Dim jc As Integer = PED.NamedCount

        For i = 0 To ic - 1

            stb.Append("  unnamedcount index   " & i & "   unnamedvalue =   " & PED.GetUnNamed(i))
        ...
        For j = 0 To jc - 1
            ...
            Try
                Dim pedtry As Object = PED.GetNamed(j).ToString

            Catch ex As Exception

                Dim pedtry As String = "tried but failed"

            End Try



            stb.Append("namedcount index " & j & "  namedkey =  " & PED.GetNamedKey(j) & "   namedvalue =   " & PED.GetNamed(j).ToString)
            stb.Append("</br>")
        Next


        stb.Append("LastChange:   " & hs.DeviceLastChange(dv.Address(hs)).ToString)


        'stb.Append(web_configdevice.buildedittable)
        'stb.Append(web_configdevice.buildwebpage)
    ...
    Public Function ConfigDevicePost(ref As Integer, data As String, user As String, userRights As Integer) As Enums.ConfigDevicePostReturn
        ...
        ''   Return web_configdevicepost.postconfig(ref, data, user, userRights)
    ...
    '        If parts("plugin") = "BuienRadarGps" Then

    '            Select Case parts("id")
    '                ''ref=8415&plugin=BuienRadarGps&instance=&action=pluginpost&name1=&lon1=&lat1=&id=dvc_bsave1&save1=Submit
    '                Case "dvc_bsave0"
    '                    street = parts("street0")
    '                    city = parts("city0")
    '                    country = parts("country0")

    '                    lon = parts("lon0")
    '                    lat = parts("lat0")

    '                    If parts("id").Length = 10 Then

    '                        index = Mid(parts("id"), 10, 1)

    '                    ElseIf parts("id").Length = 11 Then

    '                        index = Mid(parts("id"), 11, 2)

    '                    End If

    '                    Files.changefilelocations(1, index, street, city, country, lon, lat, ref, "")
    ...
    Public Sub SetIOMulti(colSend As System.Collections.Generic.List(Of HomeSeerAPI.CAPI.CAPIControl))
        ...
        For Each CC In colSend
            ...
            Dim devval As Double = hs.DeviceValue(CC.Ref)
            ...
            Where (el.<location_hsref>.Value = CC.Ref)
                            Select el.<location_index>.Value).FirstOrDefault()

            Dim size As Boolean

            If CC.ControlValue = 0 Then

                size = True

                '' hs.SetDeviceValueByRef(CC.Ref, 0, True)



            ElseIf CC.ControlValue = 100 Then

                size = False

                '' hs.SetDeviceValueByRef(CC.Ref, 100, True)

            End If

            ChangeNamedEdoHsDevice(CC.Ref, size)

            Arima(query1).SetDeviceGraphSize(CC.Ref, size)


            hs.SetDeviceLastChange(CC.Ref, Now)
            ...
            ''  Arima(query1).SetDeviceValue(CC.Ref, CC.ControlValue)
            '' SetDeviceValue(CC.Ref, CC.ControlValue)
            '' SendCommand(CC.Ref, CC.ControlValue)
...
#Region "Init"
    ...
    Public Function InitIO(ByVal port As String) As String
        ...
        'RegisterWebPage(sBuienRadarPage)
        ' '' RegisterWebPage(sRegisterConfigPage)
        'RegisterPluginWebPage(sRegisterConfigPage)
...
#Region "Web Page Processing"
    ...
    Private Function SelectPage(ByVal pageName As String) As Object
        ...
        Select Case pageName
            ...
            Case BuienRadarPage.PageName
                SelectPage = BuienRadarPage
                'Case BuienRadarConfigPage.PageName
                '    SelectPage = BuienRadarConfigPage
                'Case EmptyPage.PageName
                '    SelectPage = EmptyPage
